...
- **Calmar** = CAGR / |MDD|
- **상/하방 캡처 (월간 수익률, Morningstar 표준):**
  - 상방 = 전략_ann(b_월>0 달들) / 벤치_ann(b_월>0 달들)
  - 하방 = 전략_ann(b_월<0 달들) / 벤치_ann(b_월<0 달들)
  - ann = (∏(1+r_m))^(12/k) − 1,  k = 해당 집합의 달 수
  - 전체 누적곱(∏(1+r_m)−1) 방식은 k가 수백이면 분모가 수천 배로 폭발해 비율이 0에 수렴하므로 사용 금지
  - ※ 이 수식은 원래 정의의 "복리수익" 계산법이 모호해 표준 정의(Morningstar)로 명확화한 것. 합격 기준은 그대로이며 결과를 보고 바꾼 것이 아님.
  - 목표: 하방 < 70%, 상방 > 85%
